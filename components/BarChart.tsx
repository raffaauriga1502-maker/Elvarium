...
const BarChart: React.FC<BarChartProps> = ({ stats, width = 400, height = 250 }) => {
  const { t } = useI18n();
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto overflow-visible">
        ...
        <g transform={`translate(${padding.left}, ${padding.top})`}>
          ...
          {[0, 5, 10, 15, 20].map(val => {
             ...
             return (
              <g key={val}>
                  ...
                  <line x1="0" y1={y} x2={chartWidth} y2={y} stroke="#475569" strokeWidth="0.5" strokeDasharray="2 2" opacity="0.5" />
              </g>
             )
          })}
          ...
          {STAT_KEYS.map((key, i) => {
            ...
            const barHeight = (Math.min(value, MAX_STAT_VALUE) / MAX_STAT_VALUE) * chartHeight;
